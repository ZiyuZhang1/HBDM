...
if args.W == 1 and args.RE:
    ...
    from HBDM_RE_G import LSM
elif args.W == 2:
    from HBDM_G import LSM
    ...
    for dataset in datasets:
        for latent_dim in latent_dims:
            if args.W == 1:
                ...
                sparse_i=torch.from_numpy(np.loadtxt(r'D:\study\thesis\project\HBDM-main\data\datasets\ppi\sparse_i.txt')).long().to(device)
                # input data, link column positions with i<j
                sparse_j=torch.from_numpy(np.loadtxt(r'D:\study\thesis\project\HBDM-main\data\datasets\ppi\sparse_j.txt')).long().to(device)
            elif args.W == 2:
                sparse_i=torch.from_numpy(np.loadtxt(r'D:\study\thesis\project\HBDM-main\data\datasets\ppi\sparse_i.txt')).long().to(device)
                # input data, link column positions with i<j
                sparse_j=torch.from_numpy(np.loadtxt(r'D:\study\thesis\project\HBDM-main\data\datasets\ppi\sparse_j.txt')).long().to(device)
                sparse_w=torch.from_numpy(np.loadtxt(r'D:\study\thesis\project\HBDM-main\data\datasets\ppi\sparse_w.txt')).long().to(device)
            ...
            if args.W == 2:
                ...
                for epoch in tqdm(range(args.epochs),desc="HBDM is Running…",ascii=False, ncols=75):
                    ...
                    if epoch%1000==0:
                        ...
                        print('Loss:',(loss.item()*N)/elements)
                        if args.LP:
