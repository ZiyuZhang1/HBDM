...
root = '/novo/omdb/pds02/PDS2843/data/sprint_tid_ascvd/gzn/thesis/HBDM/data/datasets/ppi_linkpredict/'
record_path = '/novo/omdb/pds02/PDS2843/data/sprint_tid_ascvd/gzn/thesis/HBDM/results/sp/sp_lp_g.pkl'
...
def calculate_shortest_path(args):
    G, source, target,weight = args
    try:
        return 1/(nx.shortest_path_length(G, source=source, target=target,weight='weight'))
...
if __name__ == '__main__':
    ...
    args_list = [(G, source, target,'weight') for source, target in zip(test_i, test_j)]
